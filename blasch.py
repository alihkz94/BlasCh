...
#                                                                             #
# Description:                                                                #
#   BlasCh processes BLAST XML results to identify, classify, and recover     #
#   false positive chimeric sequences from metabarcoding or eDNA datasets.   #
#   Sequences are categorized into three groups: non-chimeric, absolute       #
#   chimeras, and borderline sequences, based on identity and coverage        #
#   thresholds.                                                               #
#                                                                             #
...
# Date:          2026-05-07                                                   #
# License:       MIT                                                          #
# Repository:    https://github.com/alihkz94/BlasCh                          #
###############################################################################

__version__ = "1.0.0"
__author__ = "Ali Hakimzadeh"

###############################################################################
#                                Logging Setup                                #
###############################################################################

logger = logging.getLogger("blasch")
...
def generate_blasch_readme(
    ...
    """Generate a README.txt file for the BlasCh output directory."""
    ...
    try:
        with open(readme_path, 'w') as f:
            ...
            f.write("##############################################\n")
            f.write("### Please cite BlasCh:\n")
            f.write("    Hakimzadeh A, Mikryukov V, Metsoja M, Tedersoo L, Anslan S. 2025.\n")
            f.write("    Are we throwing away good data? Evaluation of chimera detection algorithms\n")
            f.write("    on long-read amplicons reveals high false-positive rates across algorithms.\n")
            f.write("    PeerJ 13:e20456 https://doi.org/10.7717/peerj.20456\n\n")
...
def run_blast_analysis(
    ...
    the reference DB (if provided) and the per-sample self-database.
    Store XML in output_dir/xml/<base_name>_blast_results.xml.
    If self_db_dir is None, all XML files already exist - skip entirely.
    """
    ...
    if missing_xml_files:
        ...
        chimeric_files = missing_xml_files
    ...
    if reference_db_prefix:
        ...
        if check_blast_db_exists(reference_db_prefix):
            logger.info(f"Reference DB validation successful: {reference_db_prefix}")
        else:
...
# Classification thresholds (defaults; overridden by CLI arguments)
...
def classify_sequence(
    ...
    Apply classification logic:
      ...
      5. Borderline => non_chimeric (rescued)
      6. Single taxonomy, no match => borderline
      7. Multiple taxonomies, no match => chimeric
    """
    if not hits_info:
...
def parse_blast_results(args):
    ...
    trimmed_base_name = base_name.replace(".chimeras", "")
...
def merge_nonchimeric_with_rescued(nonchimeric_dir, non_chimeric_out_dir, output_dir):
    ...
    basename_non_chimeric.fasta from non_chimeric_out_dir and write combined
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            f"BlasCh v{__version__} - BLAST-based false positive chimera detection & recovery\n"
            "for metabarcoding and environmental DNA (eDNA) datasets.\n\n"
            "BlasCh uses per-sample BLAST self-databases (and optionally a reference database)\n"
            "to distinguish false positive chimeras from true sequences flagged by denoisers."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter
    )

    parser.add_argument("--version", action="version", version=f"BlasCh {__version__}")
    ...
    parser.add_argument("--self_fasta_dir", default="./",
                        help="Directory with FASTA files for building per-sample self-databases.")
    parser.add_argument("--reference_db",
                        help="Path to a reference DB prefix or reference FASTA (script will create DB if needed).")
    parser.add_argument("--output_dir", default="./blasch_output",
    ...
    parser.add_argument("--high_coverage_threshold", type=float, default=99.0,
                        help="Coverage threshold (%) for high-confidence non-chimeric classification.")
    parser.add_argument("--high_identity_threshold", type=float, default=99.0,
                        help="Identity threshold (%) for high-confidence non-chimeric classification.")
    parser.add_argument("--borderline_coverage_threshold", type=float, default=89.0,
                        help="Coverage threshold (%) for borderline => rescued non-chimeric classification.")
    parser.add_argument("--borderline_identity_threshold", type=float, default=80.0,
                        help="Identity threshold (%) for borderline => rescued non-chimeric classification.")
    ...
    logger.info(f"=== BlasCh v{__version__}: False Positive Chimera Detection & Recovery ===")
